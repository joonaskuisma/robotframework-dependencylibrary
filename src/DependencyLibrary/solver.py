...
#PROG_NAME = __name__ + ".DependencySolver"
#PROG_CALL = PROG_NAME.split(".")[-1]
PROG_CALL = "DependencySolver"
PROG_NAME = __name__.split(".")[0] + "." + PROG_CALL
...
Note that if you call '--help' like 'robot --prerunmodifier robotframework_dependencysolver.DependencySolver:-h .', you will get robot error:
...
class DependencyArgumentParser(argparse.ArgumentParser):
    def add_arguments(self) -> None:
        ...
        self.add_argument('--version', action='version', version='Running ' + repr(PROG_CALL) + ' from robotframework-dependencylibrary ' + __version__)
...
class DependencySolver(SuiteVisitor):
    ...
    def start_suite(self, suite: TestSuite) -> bool | None:
        ...
        if not self.check_done:
            ...
            try:
                # Kokeillaan lisätä myös ylimmän tason suite mukaan. Tallennettavia tietoja on turhan paljon, voisi yksinkertaistaa
